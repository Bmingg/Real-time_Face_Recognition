...
face_recognizer.read('models/trained_on_test5.yml')
...
for image_path in image_paths:
    ...
    if not faces_detected:
        ...
        for face in faces_detected:
            ...
            best_id = min(distances, key=lambda id_: distances[id_]["Correlation"])
            predicted_name = name.get(str(face_recognizer.getLabels()[best_id][0]), "Unknown")
            print(f"Image: {image_path}, Prediction: {predicted_name}, Distances: {distances[best_id]}")
