...
#     epsilon = 1e-10
#     hist1 = np.maximum(hist1, epsilon)  # Avoid division by zero
...
#     text = f'{name}:, ChiSq={distances["Chisquare"]:.2f} '
...
for image_path in image_paths:
    ...
    if not faces_detected:
        ...
        for face in faces_detected:
            ...
            best_id = min(distances, key=lambda id_: distances[id_]["Chisquare"])
            predicted_name = name.get(str(face_recognizer.getLabels()[best_id][0]), "Unknown")
            print(f"Image: {image_path} - {distances[best_id]["Chisquare"]:.2f} ({predicted_name})")