...
def compare_histograms(hist1, hist2):
    ...
    # Check if histograms are of equal length before calculating Mahalanobis distance
    # if len(hist1) == len(hist2):
    #     cov_matrix = np.cov(np.array([hist1, hist2]), rowvar=False)
    #     if np.linalg.det(cov_matrix) != 0:  # Check if covariance matrix is invertible
    #         inv_cov_matrix = np.linalg.inv(cov_matrix)
    #         distances["Mahalanobis"] = mahalanobis(hist1, hist2, inv_cov_matrix)
